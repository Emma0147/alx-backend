...
def before_request(login_as: int = None):
    ...
    print(user)
    g.user = user
    g.current_time = datetime.now(pytz.timezone(get_timezone()))
    g.current_time = g.current_time.strftime('%b %d, %Y, %I:%M:%S %p')
...
def index():
    ...
    hello world
    """
    return render_template('index.html', current_time=g.current_time)
